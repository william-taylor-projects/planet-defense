...
public class TutorialScene extends Scene {
	...
	/** A reference to the MainObjects class that holds all the objects for the main scene */
	...
	public void onExit(Integer nextScene) {
		...
		/** Reset all objects just incase the user does the tutorial again */
		imageItems.reload();
		textItems.reload();
	...
	public class TutorialObjects {		
		...
		/** just updates all objects in the scene */
		public void update() {
			...
			/** if we are at this state the following objects need to be updated*/
			if(state >= 2) {
				planet.update();
				enemy.update();
			}
		...
		public void onRender(RenderQueue renderList) {
			...
			/** The push all other objects if we are in the correct state */
			if(state >= 2) {
		...
		/** reset function just resets all objects contained in the inner class */
		public void resetObject() {
			/** remove and hide the power up */
			this.powerUp.enable();
			this.powerUp.Hide();
			
			/** Then just reset all the other objects */
