...
import objects.Ship.CURRENT_SHIP;
...
public class UpgradeState extends Scene {
	...
	/** onRender function just pushes all the objects to the queue */
	@Override
	...
	public class UpgradeButtons {
		...
		/** Basic contructor allocates all the memory needed for all buttons & events */
		...
		public void initialise(IFactory factory) {
			...
			/** setup the events for each button  */
			...
			/** The init the perk events which need to created on each play */
			initPerkEvents();
		}
		
		/** simply creates new buy perks events for each play session */
		...
		/** Handles the onTouch event which passes all the data to the events */
		public void onTouch(MotionEvent e, int x, int y) {
	...
	public class UpgradeText  {
		...
		/** The constructor that sets up the objects needed */
		public UpgradeText() {
