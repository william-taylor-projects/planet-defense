...
public class StatisticsScene extends Scene {
	/** A reference to all the scenes images */
	...
	public class StatisticObjects {
		...
		/** The function will handle touch events*/
		public void onTouch(MotionEvent e, int x, int y) {
			/** & send them to the button events */
		...
		/** update function which will update the objects */
		public void update() {
			resetButton.Update();
			background.update();
			backButton.Update();
		...
		/** onRender will add all the renderable objects to the queue */
		public void onRender(RenderQueue renderList) {
		...
		/** initialise function loads all the objects ready for updating and drawing*/
		public void initialise(IFactory factory) {
			/** get the background and back button from the framework factory */
			...
			/** Setup the back and reset events */
